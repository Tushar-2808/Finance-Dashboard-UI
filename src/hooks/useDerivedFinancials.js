...
export const useDerivedFinancials = () => {
  ...
  const totalIncome = useMemo(() => visibleTransactions.filter(t => t.kind === 'income').reduce((sum, t) => sum + t.amount, 0), [visibleTransactions]);
  const totalExpenses = useMemo(() => visibleTransactions.filter(t => t.kind === 'expense').reduce((sum, t) => sum + t.amount, 0), [visibleTransactions]);
  const currentBalance = totalIncome - totalExpenses;
  const monthlyBreakdown = useMemo(() => computeMonthlyBreakdown(transactions), [transactions]);
  ...
  const computeDelta = field => {
    ...
    if (delta > 0) return {
      text: ` ${Math.abs(delta).toFixed(1)}% vs last month`,
      trend: 'up',
      value: delta
    };
    if (delta < 0) return {
