...
const RecentTransactionsMini = ({
  ...
  if (recent.length === 0) {
    return <Card className="animate-slide-up bg-white/40 dark:bg-gray-900/40 border-dashed">
  ...
  return <div className="space-y-4">
      <div className="flex items-center justify-between px-1">
        ...
        <Link to="/transactions" className="text-[11px] font-bold text-violet-600 dark:text-violet-400 hover:text-violet-500 hover:underline transition-all uppercase tracking-wider">
          View Full Ledger →
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 gap-3">
        ...
        return <Card key={tx.id} className={cn('animate-slide-up p-0 overflow-hidden border-0 shadow-sm border-l-4 transition-all hover:translate-x-1', isIncome ? 'border-l-teal-500' : 'border-l-orange-500')}>
              <div className="flex items-center gap-3 p-3.5 bg-white dark:bg-gray-900">
                <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center shrink-0', isIncome ? 'bg-teal-50 dark:bg-teal-900/20 text-teal-600 dark:text-teal-400' : 'bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400')}>
                ...
                <div className="text-right">
                  <p className={cn('text-sm font-black whitespace-nowrap', isIncome ? 'text-teal-600' : 'text-orange-600')}>
