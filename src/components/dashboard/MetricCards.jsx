...
const MetricCards = ({
  ...
  const cards = [{
    ...
    icon: Wallet,
    color: currentBalance >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400',
    border: currentBalance >= 0 ? 'border-t-emerald-500' : 'border-t-rose-500',
    delta: null
  }, {
    label: 'Total Income',
  ...
  return <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      delta
          ...
          <div className="flex flex-col items-center text-center space-y-1">
            ...
            {delta && delta.trend !== 'none' ? <p className={cn('text-[11px] font-semibold flex items-center bg-gray-50 dark:bg-gray-800/50 px-2 py-0.5 rounded-full mt-1', delta.trend === 'up' ? 'text-emerald-500' : delta.trend === 'down' ? 'text-rose-500' : 'text-gray-500')}>
                {delta.trend === 'up' && <ArrowUp size={10} className="mr-0.5" />}
                {delta.trend === 'down' && <ArrowDown size={10} className="mr-0.5" />}
                {delta.text}
              </p> : <div className="h-5" aria-hidden="true" /> /* Spacer for alignment */}
          </div>
        </Card>)}
    </div>;
};
