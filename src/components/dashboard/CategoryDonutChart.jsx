...
const CategoryDonutChart = ({
  ...
  if (categoryTotals.length === 0) {
    return <Card className="animate-slide-up flex flex-col items-center justify-center h-full min-h-[300px]">
        <p className="text-sm font-medium text-gray-400 italic">No spending data</p>
      </Card>;
  }
  return <Card className="animate-slide-up border-0 shadow-sm relative overflow-visible">
      ...
      <div className="h-[240px] relative">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <PieChart>
            <Pie data={categoryTotals} dataKey="total" nameKey="category" cx="50%" cy="50%" innerRadius={70} outerRadius={100} paddingAngle={4} cornerRadius={8}>
              {categoryTotals.map(entry => {
              const config = categoryConfig[entry.category];
              return <Cell key={entry.category} fill={config?.hex ?? '#94a3b8'} strokeWidth={0} style={{
                filter: 'drop-shadow(0 4px 6px rgb(0 0 0 / 0.05))'
              }} />;
            })}
            </Pie>
            <Tooltip formatter={value => formatINR(Number(value))} contentStyle={{
      ...
      <div className="grid grid-cols-2 gap-x-6 gap-y-3 mt-6">
        ...
        return <div key={entry.category} className="space-y-1.5">
