...
const InsightTile = ({ topCategoryByAmount, topCategoryByCount, avgMonthlySpend, savingsRate }: InsightTileProps) => {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {tiles.map(({ label, icon: Icon, value, subValue, iconBg, iconColor, progress, progressColor }) => (
        <Card key={label} hover className="animate-slide-up border-0 shadow-sm p-5 flex flex-col h-full bg-white dark:bg-gray-900">
          ...
          <div className="mt-auto">
            ...
            {subValue && (
              <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase mt-0.5">{subValue}</p>
            )}
